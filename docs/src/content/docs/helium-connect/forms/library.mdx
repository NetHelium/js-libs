...
formulaires <a href="https://helium-connect.fr" target="_blank">Hélium Connect</a> sur vos pages 
web.
...
<Aside type="caution">
  ...
  de la librairie sera chargée automatiquement et le code utilisé pour la version majeure 
  précédente ne sera pas forcément compatible avec la nouvelle.
...
<Aside type="danger" title="Comportement en cas d'URL inattendue">
  ...
  - Si l'URL est valide mais ne correspond pas à un formulaire Hélium Connect, une iframe 
  sera générée avec le contenu de la page cible mais elle ne sera pas visible puisque la hauteur ne 
...
Les variables CSS suivantes sont définies par la librairie :
...
Vous pouvez modifier les valeurs de ces variables :
...
Les styles par défaut sont les suivants :
...
/* Iframe encapsulée dans <hc-form> */
iframe {
  border: none;
  width: 100%;
}

/* Message d'erreur encapsulé dans <hc-form> */
[role="alert"] {
  text-align: center;
...
Chaque formulaire Hélium Connect est accessible via deux urls différentes (avec ou sans le suivi de 
...
Lorsque ce type de lien est utilisé dans le contenu d'une campagne Hélium Connect, le 
